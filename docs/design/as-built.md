...
`apps/web/app/entrar/page.tsx` (server) + `sign-in-form.tsx` (client) + `entrar.module.css`. `login-card` central full-height (`surface` sobre `bg-root`, borda `line`): wordmark, título "Entrar" e o formulário OTP em duas etapas (bala-traçante da Fase 2, #190). **Passo 1** — campo de e-mail (`type=email`, `autocomplete=email`), botão "Continuar", divisor "ou" (hairline `line` dos dois lados) e "Continuar com Google" **inerte** (placeholder até #191, renderizado `disabled`). **Passo 2** — `OtpInput`, contador de expiração e os botões "Embarcar →" (submit, desabilitado até 6 dígitos) e "Trocar e-mail" (volta ao passo 1). O formulário é o único cliente: passo 1 chama o proxy `/api/otp/request`; passo 2 entrega ao provedor `otp` do Auth.js (`signIn("otp", …)`), que cunha a sessão na API interna (ADR-0004); no sucesso, navega para `/app`.
