...
- **Orçamento do grupo (salto):** painel que soma três fontes — `Pesquisa de Passagem` `Escolhida` de cada `Trajeto` (passagens), `Hospedagem` por `Parada` e `Extra`s — em **subtotais por moeda** (Plex Mono grande), com recortes por pessoa e por grupo. **Não há conversão de câmbio nem moeda-base**: moedas distintas aparecem como subtotais separados (nota mono), não como um número único. Lacunas (Trajeto sem Escolhida) aparecem como "a decidir". Só `Organizador`es editam linhas.
- **Notificações (inbox pessoal):** lista por destinatário dos `Convite`s a aceitar/recusar, `Escolhida`s marcadas em `Viagem`s suas, `Tarefa`s atribuídas a você e `menção`s — com estado lida/não-lida e contador no shell. **Distinta** da "Atividade recente" do painel (feed factual do grupo, sem estado de leitura). Preferências de entrega (por tipo + resumo por e-mail) ficam no Perfil.
